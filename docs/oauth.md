...
| `Authorization server issuer mismatch` | provider delegates auth to a different domain than the MCP host (Freshworks: freshservice.com endpoint, myfreshworks.com issuer) | upgrade mux — current versions trust the issuer the discovery document declares |
...
| `AADSTS50011` redirect-URI mismatch naming an unknown client ID | provider fronts a fixed first-party OAuth app that forbids loopback redirects (e.g. microsoft365.mcp.claude.com) | not mux-fixable — use the vendor's own connector |
