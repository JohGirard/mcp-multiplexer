...
async fn meta_tools_stay_lazy_with_dead_servers() {
    ...
    let servers = tokio::time::timeout(Duration::from_secs(5), a.list_servers())
        .await.expect("list_servers blocked on cold servers").unwrap();
    ...
    let hits = tokio::time::timeout(Duration::from_secs(5),
        a.search_tools("echo".into(), Some("mock".into()), 5))
        .await.expect("scoped search blocked on cold servers").unwrap();
    ...
    let err = a.search_tools("echo".into(), Some("nope".into()), 5).await.unwrap_err().to_string();
    assert!(err.contains("unknown server") && err.contains("mock"), "{err}");
    ...
    assert_eq!(servers.iter().find(|s| s.name == "hang").unwrap().status, "cold");
}
...
async fn describe_blocked_tool_reports_config_block() {
    ...
    let err = a.describe_tool("mock".into(), "echo".into()).await.unwrap_err().to_string();
    assert!(err.contains("blocked by config"), "{err}");
    // genuinely nonexistent tools still report as unknown
    let err = a.describe_tool("mock".into(), "nosuch".into()).await.unwrap_err().to_string();
