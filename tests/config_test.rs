...
fn parses_stdio_and_http() {
    let c: Config = serde_json::from_str(r#"{"mcpServers":{
    ...
    assert_eq!(c.mcp_servers["b"].url.as_deref(), Some("https://x.test/mcp"));
    c.validate().unwrap();
}
...
fn rejects_both_command_and_url() {
    let c: Config = serde_json::from_str(
        r#"{"mcpServers":{"bad":{"command":"x","url":"https://y"}}}"#).unwrap();
...
#[test]
fn allow_deny_glob() {
    let c: Config = serde_json::from_str(r#"{"mcpServers":{"s":{
        "command":"x","allow":["get_*","ping"],"deny":["get_secret"]}}}"#).unwrap();
...
fn no_allow_means_all_allowed_except_deny() {
    let c: Config = serde_json::from_str(
        r#"{"mcpServers":{"s":{"command":"x","deny":["nuke"]}}}"#).unwrap();
    let s = &c.mcp_servers["s"];
